...
def get_loc(message):
    ...
    button_message_2(res.status_code, message)


def button_message_2(code, message):
    if code == 404:
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, selective=False)
        item4 = types.KeyboardButton("Отправить геопозицию", request_location=True)
        item5 = types.KeyboardButton("Справка")
        markup.row(item4)
        markup.add(item5)
        bot.send_message(message.chat.id, 'Свободных мест рядом нет', reply_markup=markup)
        return
    ...
    cap = "Парковка по адресу " + "[" + data[message.chat.id]["address"] + "](" + data[message.chat.id][
        "mapServiceLink2GIS"] + ")" + "\n" + \
          "Расстояние \- *" + str(round(int(data[message.chat.id]["distance"]) / 1000, 1)).replace('.',
                                                                                                   '\.') + "* км;" + "\n" + \
...
def next_message_reply(message):
    if message.text == "Следующая":
        ...
        res = requests.get(
            url=API_URL + '/?last_camera_id=' + str(data[message.chat.id]['cameraId']) + '&longitude=' + str(
                data[message.chat.id]['coords'][1]) + '&latitude=' + str(data[message.chat.id]['coords'][0]))
        ...
        if data[message.chat.id]['prevCameraId'] != None:
            res = requests.get(
                url=API_URL + '/?last_camera_id=' + str(data[message.chat.id]['prevCameraId']) + '&longitude=' + str(
                    data[message.chat.id]['coords'][1]) + '&latitude=' + str(data[message.chat.id]['coords'][0]))
        else:
            res = requests.get(
                url=API_URL + '/?longitude=' + str(data[message.chat.id]['coords'][1]) + '&latitude=' + str(
                    data[message.chat.id]['coords'][0]))
        data[message.chat.id] = res.json()
        print(data[message.chat.id])
        r = requests.get(url=data[message.chat.id]["imgUrl"])
        cap = "Парковка по адресу " + "[" + data[message.chat.id]["address"] + "](" + data[message.chat.id][
